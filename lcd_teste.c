...
//****************************Fun��es
static task_state_t LCD_Write_Data(unsigned char RS, unsigned char Data)
{
    //*****************************************************************************
...
void DisplayLCD_Init()
{
    ...
    timer = Get_Timer_Count();
    /*  Tempo necessario para a inicializacao do LCD apos power-on */
    while(100 >= (Get_Timer_Count() - timer));
	...
	unsigned char *TextoLCD)
{
    ...
    switch (Linha)
    {
        case 1:
            ...
            while(FINISHED != LCD_Write_Data(0, Coluna));  /* Comando, visto que estamos 
            posicionando o cursor */
        break;
        
        case 2:
            Coluna = Coluna + 0xBF; //2 + 0xBF = 0XC1
            while(FINISHED != LCD_Write_Data(0, Coluna));
